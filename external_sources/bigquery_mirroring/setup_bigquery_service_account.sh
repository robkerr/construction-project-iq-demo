...
if [ $# -lt 3 ]; then
  echo "Usage: $0 <PROJECT_ID> <DATASET_ID> <SERVICE_ACCOUNT_NAME> [CUSTOM_ROLE_ID]"
  echo ""
  echo "  PROJECT_ID           - Your GCP project ID"
  echo "  DATASET_ID           - BigQuery dataset to mirror (e.g. epc_workorders)"
  echo "  SERVICE_ACCOUNT_NAME - Name for the new service account (e.g. svc-fabric-bq-mirror)"
  echo "  CUSTOM_ROLE_ID       - Optional custom IAM role ID (default: FabricBigQueryMirror)."
  echo "                         Override if the default is stuck in GCP soft-deleted state."
  exit 1
fi
...
# The custom IAM role ID can be overridden via a 4th arg or the
# FABRIC_BQ_ROLE_ID env var. This is useful when the default ID is stuck in
# GCP's soft-deleted state (an ID marked for deletion cannot be recreated for
# ~7 days and sometimes cannot be undeleted due to IAM eventual consistency).
CUSTOM_ROLE_ID="${4:-${FABRIC_BQ_ROLE_ID:-FabricBigQueryMirror}}"
...
echo "   6. Run: ./run.sh mirroring.yaml"
echo ""
