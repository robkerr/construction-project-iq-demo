...
.DESCRIPTION
    Using the service principal from .env, this script:
      1. creates (or reuses) a schema-enabled Lakehouse named $FABRIC_LAKEHOUSE_NAME
      2. uploads out/parquet/*.parquet to the Lakehouse Files/landing/ folder (OneLake DFS)
      3. imports fabric/notebooks/*.ipynb into the workspace
    Writes FABRIC_LAKEHOUSE_ID to .env.

.NOTES
    Idempotent: reuses existing items by display name.
    Requires setup_spn.ps1 to have populated the SPN_* values in .env, and
    'python .\data_gen\generate.py' to have produced out/parquet.
#>
[CmdletBinding()]
param([switch]$SkipUpload, [switch]$SkipNotebooks)
...
if ($existing) {
    ...
    $created = Invoke-FabricLro -Path "/workspaces/$ws/lakehouses" -Token $token `
        -Body @{
            displayName     = $lhName
            description     = 'Contoso E&C Project Intelligence Lakehouse (synthetic SAP + non-SAP data)'
...
if (-not $SkipNotebooks) {
    ...
    $nbDir = Join-Path $root 'fabric\notebooks'
    $nbFiles = Get-ChildItem (Join-Path $nbDir '*.ipynb') -ErrorAction SilentlyContinue | Sort-Object Name
    if (-not $nbFiles) {
        Write-Warning "  No .ipynb found. Run: python .\fabric\notebooks\build_notebooks.py"
    }
