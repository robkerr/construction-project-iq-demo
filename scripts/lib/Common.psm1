<#
.SYNOPSIS
    Shared helpers for the Contoso E&C Project-Intelligence Fabric demo scripts:
    - .env loading / saving
    - Azure + Fabric access-token acquisition (SPN or signed-in az user)
    - Thin Fabric REST wrappers (items, long-running operations, OneLake upload)

.NOTES
    API surface and versions are the proven set used across our Fabric demos:
    ...
    Keep these stable unless a script fails against a newer API version.
#>
...
function Get-DotEnvPath { return (Join-Path (Get-RepoRoot) '.env') }
...
function Set-DotEnvValue {
    <# Upserts a key=value into .env (creates the file if needed). #>
    [CmdletBinding()]
    param(
...
function Get-FabricToken {
    <#
      Bearer token for the Fabric API. Uses the SPN from .env when present, otherwise the
      signed-in az user. Fabric API audience: https://api.fabric.microsoft.com
    #>
    [CmdletBinding()]
    param([switch]$UseSpn)
    $env = Import-DotEnv
...
function Invoke-FabricApi {
    ...
    param(
        ...
        [Parameter(Mandatory)][string]$Path,   # e.g. /workspaces/{id}/lakehouses
        [object]$Body,
        [string]$Token,
        [switch]$UseSpn
    )
...
function Send-OneLakeFile {
    <#
      Uploads a local file to OneLake via the ADLS Gen2 DFS API (create -> append -> flush).
      Good for the small demo parquet files.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$WorkspaceId,
        [Parameter(Mandatory)][string]$LakehouseId,
        [Parameter(Mandatory)][string]$RelativePath,   # e.g. Files/landing/dim_project.parquet
...
function Invoke-FabricLro {
    <#
      POST that may return a 202 long-running operation. Polls Operation-Location / Location
      until the operation succeeds, then returns the final response.
    #>
    [CmdletBinding()]
    param(
