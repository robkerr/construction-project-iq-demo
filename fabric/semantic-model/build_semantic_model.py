...
COLS = {
    ...
    "dim_rfq": [
        ("rfq_id", "string", "rfq_id", False, None),
        ("project_id", "string", "project_id", True, None),
        ("wbs_id", "string", "wbs_id", True, None),
        ("equipment_tag", "string", "equipment_tag", False, None),
        ("material_category", "string", "material_category", False, None),
        ("equipment_desc", "string", "equipment_desc", False, None),
        ("engineers_estimate", "double", "engineers_estimate", False, "none"),
        ("currency", "string", "currency", False, None),
        ("issued_date", "dateTime", "issued_date", False, None),
        ("bids_due_date", "dateTime", "bids_due_date", False, None),
        ("required_on_site", "dateTime", "required_on_site", False, None),
        ("status", "string", "status", False, None),
        ("bidder_count", "int64", "bidder_count", False, "none"),
        ("origin_system", "string", "origin_system", False, None),
    ],
    "dim_tech_requirement": [
        ("req_id", "string", "req_id", False, None),
        ("material_category", "string", "material_category", False, None),
        ("requirement", "string", "requirement", False, None),
        ("required_value", "string", "required_value", False, None),
        ("unit", "string", "unit", False, None),
        ("weight", "int64", "weight", False, "none"),
        ("is_mandatory", "boolean", "is_mandatory", False, None),
        ("origin_system", "string", "origin_system", False, None),
    ],
    "fact_bid": [
        ("bid_id", "string", "bid_id", False, None),
        ("rfq_id", "string", "rfq_id", True, None),
        ("project_id", "string", "project_id", True, None),
        ("wbs_id", "string", "wbs_id", True, None),
        ("equipment_tag", "string", "equipment_tag", False, None),
        ("material_category", "string", "material_category", False, None),
        ("supplier_id", "string", "supplier_id", True, None),
        ("supplier_name", "string", "supplier_name", False, None),
        ("bid_date", "dateTime", "bid_date", False, None),
        ("currency", "string", "currency", False, None),
        ("quoted_price", "double", "quoted_price", False, "none"),
        ("spares_price", "double", "spares_price", False, "none"),
        ("freight_price", "double", "freight_price", False, "none"),
        ("delivery_weeks", "int64", "delivery_weeks", False, "none"),
        ("weeks_late", "int64", "weeks_late", False, "none"),
        ("payment_advance_pct", "int64", "payment_advance_pct", False, "none"),
        ("warranty_months", "int64", "warranty_months", False, "none"),
        ("incoterms", "string", "incoterms", False, None),
        ("technical_score", "double", "technical_score", False, "none"),
        ("tech_compliant_count", "int64", "tech_compliant_count", False, "none"),
        ("tech_deviation_count", "int64", "tech_deviation_count", False, "none"),
        ("tech_exception_count", "int64", "tech_exception_count", False, "none"),
        ("tbe_status", "string", "tbe_status", False, None),
        ("is_technically_qualified", "boolean", "is_technically_qualified", False, None),
        ("commercial_deviation_count", "int64", "commercial_deviation_count", False, "none"),
        ("evaluated_price", "double", "evaluated_price", False, "none"),
        ("price_loading", "double", "price_loading", False, "none"),
        ("cbe_rank", "int64", "cbe_rank", False, "none"),
        ("award_status", "string", "award_status", False, None),
        ("recommended", "boolean", "recommended", False, None),
        ("origin_system", "string", "origin_system", False, None),
    ],
    "fact_bid_tech_eval": [
        ("eval_id", "string", "eval_id", False, None),
        ("bid_id", "string", "bid_id", True, None),
        ("rfq_id", "string", "rfq_id", True, None),
        ("supplier_id", "string", "supplier_id", True, None),
        ("req_id", "string", "req_id", True, None),
        ("requirement", "string", "requirement", False, None),
        ("required_value", "string", "required_value", False, None),
        ("quoted_value", "string", "quoted_value", False, None),
        ("compliance", "string", "compliance", False, None),
        ("is_mandatory", "boolean", "is_mandatory", False, None),
        ("comment", "string", "comment", False, None),
        ("origin_system", "string", "origin_system", False, None),
    ],
}
...
MEASURES = {
    ...
    "dim_project": [
        ...
        {"name": "Worst Schedule Risk", "fmt": "#,##0.0",
         "desc": "Portfolio KPI: the single highest project Schedule Risk Score (the worst project) in the current filter context.",
         "dax": "MAXX(VALUES(dim_project[project_id]), [Schedule Risk Score])"},
    ...
    "fact_bid": [
        {"name": "Bid Count", "fmt": "#,##0",
         "desc": "Number of supplier bids in context (across the selected RFQ/category/project).",
         "dax": "COUNTROWS(fact_bid)"},
        {"name": "Quoted Price", "fmt": "$#,##0",
         "desc": "CBE: quoted base price summed in context (at supplier grain = that bid's quote).",
         "dax": "SUM(fact_bid[quoted_price])"},
        {"name": "Evaluated Price", "fmt": "$#,##0",
         "desc": "CBE: normalized evaluated price summed in context (at supplier grain = that bid's evaluated cost, incl. disqualified bids).",
         "dax": "SUM(fact_bid[evaluated_price])"},
        {"name": "Qualified Bids", "fmt": "#,##0",
         "desc": "TBE: bids that passed technical evaluation (no mandatory exception, score >= 70).",
         "dax": "CALCULATE(COUNTROWS(fact_bid), fact_bid[is_technically_qualified] = TRUE())"},
        {"name": "Disqualified Bids", "fmt": "#,##0",
         "desc": "TBE: technically non-compliant bids (excluded from commercial ranking).",
         "dax": "CALCULATE(COUNTROWS(fact_bid), fact_bid[is_technically_qualified] = FALSE())"},
        {"name": "Avg Technical Score", "fmt": "#,##0.0",
         "desc": "TBE: average weighted technical compliance score (0-100) of bids in context.",
         "dax": "AVERAGE(fact_bid[technical_score])"},
        {"name": "Technical Deviations", "fmt": "#,##0",
         "desc": "TBE: total number of technical deviations across bids in context.",
         "dax": "SUM(fact_bid[tech_deviation_count])"},
        {"name": "Lowest Quoted Price", "fmt": "$#,##0",
         "desc": "CBE: lowest quoted base price received (may be a disqualified bid).",
         "dax": "MIN(fact_bid[quoted_price])"},
        {"name": "Lowest Evaluated Price", "fmt": "$#,##0",
         "desc": "CBE: lowest normalized evaluated price among technically-qualified bids.",
         "dax": "CALCULATE(MIN(fact_bid[evaluated_price]), fact_bid[is_technically_qualified] = TRUE())"},
        {"name": "Recommended Evaluated Price", "fmt": "$#,##0",
         "desc": "CBE: evaluated price of the recommended award (lowest evaluated qualified bid).",
         "dax": "CALCULATE(SUM(fact_bid[evaluated_price]), fact_bid[recommended] = TRUE())"},
        {"name": "Recommended Quoted Price", "fmt": "$#,##0",
         "desc": "CBE: quoted price of the recommended award.",
         "dax": "CALCULATE(SUM(fact_bid[quoted_price]), fact_bid[recommended] = TRUE())"},
        {"name": "Recommended Supplier", "fmt": None,
         "desc": "CBE: supplier name of the recommended award in the current context (single RFQ).",
         "dax": "CALCULATE(SELECTEDVALUE(fact_bid[supplier_name]), fact_bid[recommended] = TRUE())"},
        {"name": "Evaluated vs Lowest Quote", "fmt": "$#,##0",
         "desc": "CBE headline: recommended evaluated price minus the lowest quoted price — the cost of choosing the compliant, best-value bid over the cheapest quote.",
         "dax": "[Recommended Evaluated Price] - [Lowest Quoted Price]"},
    ],
    "dim_rfq": [
        {"name": "RFQ Count", "fmt": "#,##0",
         "desc": "Number of bid packages (RFQs) in context.",
         "dax": "DISTINCTCOUNT(dim_rfq[rfq_id])"},
        {"name": "Engineers Estimate", "fmt": "$#,##0",
         "desc": "Sum of the engineer's cost estimate across RFQs in context.",
         "dax": "SUM(dim_rfq[engineers_estimate])"},
    ],
    "fact_bid_tech_eval": [
        {"name": "Compliance Items", "fmt": "#,##0",
         "desc": "TBE: count of technical requirement line items in context (used in the compliance matrix).",
         "dax": "COUNTROWS(fact_bid_tech_eval)"},
        {"name": "Deviation Items", "fmt": "#,##0",
         "desc": "TBE: count of requirement line items evaluated as a deviation.",
         "dax": "CALCULATE(COUNTROWS(fact_bid_tech_eval), fact_bid_tech_eval[compliance] = \"Deviation\")"},
        {"name": "Exception Items", "fmt": "#,##0",
         "desc": "TBE: count of requirement line items evaluated as an exception (non-compliant).",
         "dax": "CALCULATE(COUNTROWS(fact_bid_tech_eval), fact_bid_tech_eval[compliance] = \"Exception\")"},
    ],
}
...
RELATIONSHIPS = [
    ...
    # Bid evaluation star: dim_project <- dim_rfq <- fact_bid <- fact_bid_tech_eval, plus
    # dim_tech_requirement -> fact_bid_tech_eval and sap_supplier <- fact_bid. (No rfq/bid ->
    # dim_wbs relationship on purpose, to avoid an ambiguous second path to dim_project.)
    ("rel_project_rfq", "dim_rfq", "project_id", "dim_project", "project_id"),
    ("rel_rfq_bid", "fact_bid", "rfq_id", "dim_rfq", "rfq_id"),
    ("rel_supplier_bid", "fact_bid", "supplier_id", "sap_supplier", "supplier_id"),
    ("rel_bid_tech_eval", "fact_bid_tech_eval", "bid_id", "fact_bid", "bid_id"),
    ("rel_req_tech_eval", "fact_bid_tech_eval", "req_id", "dim_tech_requirement", "req_id"),
]
